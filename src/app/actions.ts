...
export async function quoteAction(input: {
  ...
  /** A recovery coupon, reported on but never spent by a quote. */
  couponCode?: string;
}): Promise<QuoteResult> {
  ...
  try {
    ...
    const quote = await apiRequest<ApiQuote>("/api/v1/checkout/quote", {
      ...
      body: {
        ...
        /* Omitted rather than sent empty: the API schema is `.strict()` and an
           empty string is a code that will never match, which would report a
           refusal at every shopper who has not typed one. */
...
export interface CheckoutInput {
  ...
  /** The recovery coupon being spent, if the shopper has one. */
 ...
 *
 * The customer tapped a WhatsApp message about a basket they left. This reads
 * back what was in it so the storefront can put it in front of them again.
 *
 * WHAT IT DOES NOT RETURN
 * Their name, phone or address — the API deliberately withholds them. WhatsApp
 * messages get forwarded, so a link that filled in a home address would hand it
 * to whoever received the forward. The form opens empty and the customer types
 * their own number, as they were going to anyway.
 *
 * Prices are absent too: the lines carry ids and quantities, and everything the
 * customer is charged is priced from the live catalogue by the quote endpoint.
 * A week-old basket cannot resurrect a week-old price.
 */
export async function resumeCartAction(
  leadId: string,
): Promise<{ ok: true; lines: CartLineInput[] } | { ok: false; error: string }> {
  try {
    const result = await apiRequest<{
      items: { productId: string; variantId: string | null; quantity: number }[];
    }>(`/api/v1/checkout/resume/${leadId}`, {
      headers: forwardClientHints(await headers()),
      /* `revalidate: 0` is how this client says no-store. The link is
         per-customer and the basket behind it is theirs; a shared cache entry
         would serve one shopper's cart to the next person opening a resume
         link. */
      revalidate: 0,
    });
    ...
    /* A dead link is the normal case here, not an incident: the lead may have
       been deleted, or the shop cleared the list. Logged quietly, and the page
       sends the customer to the shop rather than showing them a stack of
       apology they cannot act on. */
    logFailure("resume link lookup failed", error);
    return { ok: false, error: toUserMessage(error) };
  }
}
